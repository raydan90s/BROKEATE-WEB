...
export interface Mensaje {
  ...
  modelo?: string;
  /** La ruta del router: "bancario" | "mixto" | "externo" | "rechazo". */
...
/** Tres puntitos que laten mientras el asistente "escribe" (CSS, ex-Animated). */
function TypingDots() {
  return (
    <View className="flex-row items-center gap-1 py-1">
      {[0, 1, 2].map((i) => (
        <span
          key={i}
          className="punto-typing h-2 w-2 rounded-full bg-text-muted"
        ...
        >
          {mensaje.pending ? (
            ...
            <>
              ...
              <Text
                className={`text-body whitespace-pre-line ${mensaje.error ? 'text-state-error' : 'text-text-primary'}`}
              >
                {mensaje.texto}
              </Text>
            </>
