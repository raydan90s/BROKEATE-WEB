...
/** El horizonte de la simulación: estima los fondos. */
const PLAZOS = [
 ...
 * **Ningún USD se calcula aquí**: cada cambio pide de nuevo el catálogo y Postgres devuelve
          ...
          <>
            {destacada ? (
              ...
              <Tarjeta>
                <Text className="text-body text-text-secondary">
                  Ninguna opción elegible para tu perfil con ese monto. Toca cualquiera de la
                  lista para ver qué pasaría, o prueba con un monto mayor.
                </Text>
              </Tarjeta>
            )}
            ...
            <Tarjeta className="gap-0 p-0">
              <View className="gap-3 p-4 pb-3">
                ...
                <ScrollView horizontal contentContainerClassName="gap-2 pr-4">
                  <Chip
                  ...
                  />
                  {bancos.map((b) => (
                    <Chip key={b} etiqueta={b} activo={banco === b} onPress={() => setBanco(b)} />
                  ))}
                </ScrollView>
              </View>
...
function Chip({ etiqueta, activo, onPress }: { etiqueta: string; activo: boolean; onPress: () => void }) {
  return (
    ...
    >
      <Text className={`text-caption font-bold ${activo ? 'text-text-onPrimary' : 'text-brand-mid'}`}>
        {etiqueta}
      </Text>
    </Touchable>
  );
...
/**
 * La tarjeta grande: lo que el motor recomienda o lo que el usuario eligió. Si es algo que
 * su perfil no admite o cuyo mínimo no alcanza, los números se muestran igual, con la regla
 * que lo bloquea encima.
 */
  ...
  return (
    <View
      className={`overflow-hidden rounded-2xl border-2 ${
        bloqueada ? 'border-state-warning' : 'border-brand-primary'
      }`}
    >
      <View
        className={`flex-row items-center gap-2 px-4 py-2.5 ${
          bloqueada ? 'bg-state-warning' : 'bg-brand-primary'
        }`}
      >
        ...
        {esSeleccion ? (
          <Touchable onPress={onQuitarSeleccion}>
      ...
      <View className="gap-4 bg-surface-background p-4">
        <View className="flex-row justify-between">
          <View>
            ...
            <Text className="text-hero font-bold text-text-primary">{usd(tasa.monto_final)}</Text>
          </View>
        ...
        <View className="flex-row gap-3">
          <View className="flex-1 items-center rounded-xl bg-brandAlpha-primarySoft p-3">
            <Text className="text-caption font-bold tracking-wider text-text-muted">CAPITAL</Text>
  ...
  return (
    <Touchable
      ...
      className={`flex-row items-center gap-3 border-t border-surface-border p-4 ${
        elegida ? 'bg-brandAlpha-primarySoft' : ''
      }`}
    >
      <Ionicons
      ...
      <View className={`flex-1 ${bloqueada && !elegida ? 'opacity-60' : ''}`}>
        <View className="flex-row items-center gap-1.5">
          <Text className="shrink text-body font-bold text-text-primary" numberOfLines={1}>
            {tasa.producto}
          </Text>
          {tasa.recomendado ? (
            <View className="rounded-md bg-stateAlpha-successSoft px-1.5 py-0.5">
              <Text className="text-caption font-bold text-state-success">Recomendada</Text>
            </View>
          ) : null}
        </View>
