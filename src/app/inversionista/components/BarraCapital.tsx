      ...
      {vacia ? (
        ...
        <View className="h-3 flex-row overflow-hidden rounded-full bg-surface-secondary">
          {conMonto.map((s) => (
            <View
              key={s.session_id}
              style={{ flexGrow: s.monto, flexShrink: 1, flexBasis: 0, backgroundColor: COLOR_PERFIL[s.perfil] }}
            />
          ))}
          {/* El hueco: lo que el cliente tiene declarado y no invertido. */}
          {hueco > 0 ? (
            <View style={{ flexGrow: hueco, flexShrink: 1, flexBasis: 0 }} className="bg-surface-divider" />
          ) : null}
        </View>
      )}
